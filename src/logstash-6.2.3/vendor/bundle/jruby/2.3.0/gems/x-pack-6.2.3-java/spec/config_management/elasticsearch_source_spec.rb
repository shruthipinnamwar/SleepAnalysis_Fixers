...
shared_examples "ssl flag is set" do
    it "is truthy" do
      expect(subject.es_options["ssl"]).to be_truthy
    end
end

shared_examples "ssl flag is not set" do
  it "is falsey" do
    expect(subject.es_options["ssl"]).to be_falsey
  end
end
...
"{
  ...
  describe "#pipeline_configs" do
    ...
    before do
      ...
      allow(mock_license_client).to receive(:get).with('_xpack').and_return(LogStash::Json.load(valid_xpack_response))
      allow(mock_license_client).to receive(:get).with('_xpack/license').and_return(LogStash::Json.load(license_response))
    ...
    context "with one `pipeline_id` configured" do
      ...
      context 'when the license server is not available' do
        ...
        before :each do
          allow(mock_license_client).to receive(:get).with('_xpack/license').and_raise("An error is here")
