  ...
  context "integration" do
    ...
    context 'after the pipeline is setup' do
      before do
        ...
        allow(subject).to receive(:get_current_license_state).and_return({:licensed => true})
    ...
    describe "#update" do
      before :each do
        ...
        allow(subject).to receive(:setup_license_checker)
        allow(subject).to receive(:es_options_from_settings_or_modules).and_return(es_options)
        allow(subject).to receive(:get_current_license_state).and_return({:state => :ok, :log_level => :info, :log_message => 'Monitoring License OK'})
        allow(subject).to receive(:exec_timer_task)
        allow(subject).to receive(:sleep_till_stop)
        setup_pipeline
      ...
      describe 'with licensing' do
        ...
        context 'when the license has expired' do
          ...
          before :each do
            ...
            expect(mock_license_client).to receive(:get).with('_xpack/license').and_return(license_response)
            setup_pipeline
            subject.update(collector.snapshot_metric)
          end
